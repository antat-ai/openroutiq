...
def _validate_no_public_credentials() -> int:
    ...
    for directory, directory_names, filenames in os.walk(ROOT):
        ...
        for filename in filenames:
            ...
            if filename == ".env":
                continue
            if path.suffix.casefold() not in _PUBLIC_TEXT_SUFFIXES and filename != ".env.example":
                continue
            checked += 1
...
def validate(tag: str | None = None) -> dict[str, str]:
    ...
    citation_version = _single(r"^version:\s*(.+)$", citation, "CITATION.cff version")
    ...
    changelog_date = _single(
        rf"^##\s+{re.escape(package_version)}\s+-\s+(\d{{4}}-\d{{2}}-\d{{2}})\s*$",
        changelog,
        "matching changelog release",
    )

    versions = {package_version, runtime_version, citation_version}
    if len(versions) != 1:
        raise ValueError(
            "version mismatch: "
            f"pyproject={package_version}, runtime={runtime_version}, citation={citation_version}"
        )
    if citation_date != changelog_date:
        raise ValueError(
            f"release-date mismatch: changelog={changelog_date}, citation={citation_date}"
        )
    ...
    if f"OpenRoutiQ {release_series} release candidate" not in constraints:
