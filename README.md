...
**Modern AI stacks expose many models, providers, deployments, modalities, and reasoning
levels. Hardcoding one model, or even a fixed model for each task, is increasingly brittle,
expensive, and wrong as workloads and model fleets change.**

OpenRoutiQ lets a local, explainable router choose the model, provider, and reasoning
effort each request actually needs. It reads the whole request, learns task distributions
from catalog examples, filters variants that cannot do the job, and ranks the rest by
expected quality, latency, and cost.

Every model/reasoning-level combination is a separate variant. Optional outcome learning
updates its view from measured quality, latency, cost, failures, and drift; operator-controlled
promotion gates keep unvalidated variants out of normal automatic routing. Developers can
accept automatic behavior or override it with policies, constraints, and explicit pins.

Release profile: OpenRoutiQ ships a dependency-free Python routing core, calibrated
request-by-model selection intelligence, risk-aware and adaptive policies, provider-native
request plans, policy-controlled model dispatch, a JSON CLI, and an optional
OpenAI-compatible LiteLLM proxy. The package is release-ready for application integration and
controlled internal deployments; internet-facing multi-tenant deployments should place the
proxy behind authenticated TLS termination with tenant quotas, shared state, backups, monitoring,
and an incident-tested rollback path.
...
Copy only the credentials you need from [`.env.example`](.env.example). The routing core does
not read provider keys or make network calls.
...
See [CONTRIBUTING.md](CONTRIBUTING.md), [SECURITY.md](SECURITY.md), the [Code of Conduct](CODE_OF_CONDUCT.md), and [CHANGELOG.md](CHANGELOG.md). Never include provider keys, private prompts, or unlicensed benchmark data in an issue or pull request.
